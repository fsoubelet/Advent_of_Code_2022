...
import std/[algorithm, math, sequtils, sugar]
import strutils as str

type
  Elf = object
    calories: Natural  # also ensures it is positive

proc elves_and_their_calories(inputs: seq[string]): seq[Elf] =
    var elves: seq[Elf] = @[]
    var current: Elf = Elf()
    ...
    for line in inputs:
        if line == "":
            ...
            let calorie = str.parseInt(line)
            current.calories = current.calories + calorie
    return elves
...
var elves: seq[Elf] = elves_and_their_calories(inputs)
var calories = collect(newSeq):
  for item in elves: item.calories

calories.sort()  # sorts inplace, increasing order
echo "Part 1: ", calories[^1]  # last element is highest

# Now the solution to part 2
let top_three_combined = calories[^3..^1].sum
echo "Part 2: ", top_three_combined