...
proc compartment_items(items_line: string): (string, string) =
    # """Return the items in each compartment of a rucksack. Each half of the line is a compartment."""
    let half_length = floorDiv(items_line.len, 2)
    return (items_line[0..half_length-1], items_line[half_length..^1])


proc common_item(rucksack1: string, rucksack2: string): char =
    # """Return the items that appear in both compartments."""
    let s1 = rucksack1.toHashSet()
    let s2 = rucksack2.toHashSet()
    return (s1 * s2).toSeq()[0]


let inputs = readFile("input.txt").splitLines()
var total_score: int = 0
var common_chars: seq[char] = @[]

for line in inputs:
    let (rucksack1, rucksack2) = compartment_items(line)
    let common_char = common_item(rucksack1, rucksack2)
    common_chars.add(common_char)

for char in common_chars:
    total_score += alphabet.find(char) + 1 # +1 because index starts at 0
...
proc common_items_three_rucksacks(rucksack1: string, rucksack2: string, rucksack3: string): char =
    # """Return the items that appear in both compartments."""
    let s1 = rucksack1.toHashSet()
    let s2 = rucksack2.toHashSet()
    let s3 = rucksack3.toHashSet()
    return (s1 * s2 * s3).toSeq()[0] # ok as there is only one
...
for line in inputs:
    rucksacks.add(line)
    if rucksacks.len == 3:
        let common_char = common_items_three_rucksacks(rucksacks[0], rucksacks[1], rucksacks[2])
        priorities_sum += alphabet.find(common_char) + 1 # +1 because index starts at 0
        rucksacks = @[] # we reset the rucksacks

echo "Part 2: ", priorities_sum
