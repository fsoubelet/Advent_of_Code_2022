...
proc all_different(elements: string): bool =
    # """Returns True if all elements are different."""
    let elemset: HashSet[char] = toHashSet(elements)
    return elemset.len == elements.len


# ----- Running ----- #

let buffer = readFile("input.txt").splitLines()[0]

# Part 1
var j = 0
for i in 4 ..< buffer.len:
    let content = buffer[i - 4 ..< i]
    if all_different(content):
        j = i
        break
echo "Part 1: ", j

# Part 2 - this is the same but with 14 characters
var k = 0
for i in 14 ..< buffer.len:
    let content = buffer[i - 14 ..< i]
    if all_different(content):
        k = i
        break
echo "Part 2: ", k
