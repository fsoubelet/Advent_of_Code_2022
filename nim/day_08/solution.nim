...
# Read the grid and start counter for each part
var
    grid = readFile("input.txt").strip.splitLines.mapIt(it.toSeq.mapIt(parseInt($it)))
    visible_trees = 0
    highest_scenic_score = 0


proc analyzeTrees(tree: int, treesUp: seq[int], treesDown: seq[int], treesLeft: seq[int], treesRight: seq[int]): int =
    # Perform both part 1 and part 2 in one pass
    var
        dist = [0, 0, 0, 0] # number of visible in each direction
        scenic_score: int   # the scenic score for this tree
    ...
    visible_trees.inc(result)

    scenic_score = foldl(dist, a*b)
    if scenic_score > highest_scenic_score:
        highest_scenic_score = scenic_score


# Iterate over the grid and count the visible trees + the scenic distance
for row in 0..grid.high:
    for col in 0..grid[0].high:
        if row in [0, grid.high] or col in [0, grid[0].high]: # these are the trees at the edge, they are always visible
            inc visible_trees
        else:
            discard analyzeTrees(grid[row][col],
                                 collect(for x in countdown(row-1, 0): grid[x][col]),
                                 collect(for x in row+1..grid.high: grid[x][col]),
                                 collect(for y in countdown(col-1, 0): grid[row][y]),
                                 grid[row][col+1..grid[0].high]
                )


echo "Part 1: ", visible_trees
echo "Part 2: ", highest_scenic_score
