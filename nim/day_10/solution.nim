...
for instruction in inputs:
    ...
    if startsWith(instruction, "addx"):
        if floorMod(cycle, 40) == 20:
            part1_result = part1_result + cycle * X
...
for instruction in inputs:
    if X <= floorMod(cycle, 40) and floorMod(cycle, 40) <= X + 2:
        row.add("#")
    else:
        row.add(" ")
    cycle = cycle + 1
    ...
    if startsWith(instruction, "addx"):
        if floorMod(cycle, 40) == 20: # Needs to also be checked here as with addx we have 2 cycles
            part1_result = part1_result + cycle * X
        elif floorMod(cycle, 40) == 1:
            crt.add(row)
            row = ""
        if X <= floorMod(cycle, 40) and floorMod(cycle, 40) <= X+2:
            row.add("#")
