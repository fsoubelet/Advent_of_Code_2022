...
def compartment_items(items_line: str) -> tuple[str, str]:
    """Return the items in each compartment of a rucksack. Each half of the line is a compartment."""
    half = len(items_line) // 2
    return items_line[:half], items_line[half:]


def common_item(compartment_1: str, compartment_2: str) -> set[str]:
    """Return the items that appear in both compartments."""
    return list(set(compartment_1) & set(compartment_2))[0]  # ok as there is only one
...
if __name__ == "__main__":
    rucksacks = Path("input.txt").read_text().splitlines()
    commons = []
    for rucksack in rucksacks:
        compartments = compartment_items(rucksack)
        commons.append(common_item(*compartments))
    print("Part 1: ", sum(letter_to_priority[common] for common in commons))

    commons_for_teams = []
    # This groups the lines by groups of 3
    groups = [rucksacks[n : n + 3] for n in range(0, len(rucksacks), 3)]
    for group in groups:
        commons_for_teams.append(common_items_three_rucksacks(*group))
    print("Part 2: ", sum(letter_to_priority[common] for common in commons_for_teams))
