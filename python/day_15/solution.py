...
Sensors and beacons always exist at integer coordinates. Each sensor knows its own position and can determine the position of a beacon precisely; however, sensors can only lock on to the one beacon closest to the sensor as measured by the Manhattan distance.
...
if __name__ == "__main__":
    ...
    sensors = [
        parse("Sensor at x={:d}, y={:d}: closest beacon is at x={:d}, y={:d}", line) for line in inputs
    ]

    # Part 1 can be solved by identifying the intervals that each sensor covers along y=2000000.
    ivals = []
    us = []

    # Calculate distances, and then the intervals where the beacon could be
    for sensor_x, sensor_y, beacon_x, beacon_y in sensors:
        distance = abs(sensor_x - beacon_x) + abs(sensor_y - beacon_y) - abs(sensor_y - 2000000)
        if distance >= 0:
            ivals.append((sensor_x - distance, sensor_x + distance))

    for low, high in sorted(ivals):
        if us and us[-1][1] >= low - 1:
            us[-1][1] = max(us[-1][1], high)
        else:
            us.append([low, high])

    print("Part 1:", sum(hi - lo for lo, hi in us))

    # Part 2 - we use the z3 solver, it's dark magic!
    s = z3.Solver()
    x, y = z3.Int("x"), z3.Int("y")

    # Constraints: x and y must be in the range 0-4000000
    s.add(0 <= x, x <= 4000000)
    s.add(0 <= y, y <= 4000000)
    ...
    for sensor_x, sensor_y, beacon_x, beacon_y in sensors:
        ...
        s.add(z3_abs(sensor_x - x) + z3_abs(sensor_y - y) > distance)

    assert s.check() == z3.sat  # check we reach a solution
    model = s.model()

    print("Part 2, coordinates of beacon:", model)
    print("Part 2, solution:", model[x].as_long() * 4000000 + model[y].as_long())
